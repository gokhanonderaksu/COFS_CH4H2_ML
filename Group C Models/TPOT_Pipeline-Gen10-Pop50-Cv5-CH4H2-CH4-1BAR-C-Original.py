...
tpot_data = pd.read_csv('C:/Users/gaksu/Desktop/github/Raw Data/HypoCOF-CH4H2-CH4-1bar-TPOT-Input-C- Original.csv', sep=',', dtype=np.float64)
features = tpot_data.drop('NCH4 - 1 bar (mol/kg)', axis=1)
training_features, testing_features, training_target, testing_target = \
            train_test_split(features, tpot_data['NCH4 - 1 bar (mol/kg)'],train_size=0.80, test_size=0.20, random_state=42)

# Average CV score on the training set was: -0.007560650166992766
exported_pipeline = make_pipeline(
    StackingEstimator(estimator=SGDRegressor(alpha=0.001, eta0=1.0, fit_intercept=False, l1_ratio=0.0, learning_rate="constant", loss="huber", penalty="elasticnet", power_t=0.5)),
    ...
    XGBRegressor(learning_rate=0.5, max_depth=4, min_child_weight=14, n_estimators=100, n_jobs=1, objective="reg:squarederror", subsample=1.0, verbosity=0)
)
...
X_unseen = pd.read_csv('C:/Users/gaksu/Desktop/KodSystems/TPOT/CH4-H2/Unseen - Class 3 - Subset - CH4.csv', sep=',')
